...
import { uid, formatMs, isLikelyNonSpeech, type TranscriptSegment } from "@nexus/core";
...
export function Overlay() {
  ...
  const [sttStatus, setSttStatus] = useState<"idle" | "listening" | "transcribing" | "error">("idle");
  ...
  useEffect(() => {
    const unlisten = listen<string>("nexus://audio-error", (event) => {
      setSttStatus("error");
      setSttErrorMessage(`Audio Device Error: ${event.payload}`);
    });
    return () => {
      void unlisten.then((fn) => fn());
    };
  ...
  useEffect(() => {
    const unlisten = listen<{ source: "microphone" | "system"; peak: number }>(
      "nexus://audio-level",
      (event) => {
        if (event.payload.source === "microphone") {
          setMicLevel(event.payload.peak);
        } else {
          setSystemLevel(event.payload.peak);
        }
      },
    );
    return () => {
  ...
  useEffect(() => {
    const unlisten = listen<{
      ...
      try {
        ...
        // Do not let coughs, throat-clearing, chimes, or other STT noise artifacts
        // enter transcript state and trigger a new answer search.
        const lower = text.toLowerCase().replace(/[^a-z0-9\s]/g, "");
        if (
          text.startsWith("[") && text.endsWith("]") ||
          text.startsWith("(") && text.endsWith(")") ||
          isLikelyNonSpeech(text) ||
          ["u y", "u", "y", "th", "shh", "ah", "oh"].includes(lower) ||
          lower.length < 3
        ) {
          setSttStatus("listening");
          return;
        ...
        setSttStatus("error");
        setSttErrorMessage(err.message || "Transcription failed");
        setTimeout(() => {
          setSttStatus(useStore.getState().listening ? "listening" : "idle");
          setSttErrorMessage(null);
        }, 8000);
      }
    });
  ...
  return (
    <div className={`flex h-screen flex-col p-2 relative ${resizeModeActive ? "" : "overlay-cursor-hidden"}`}>
      ...
      <section
        ...
        style={{
          // Apply opacity only to the background, keep text bright
          backgroundColor: `rgba(12, 14, 20, ${0.72 * (settings.stealth.opacity ?? 0.92)})`,
        }}
      >
        ...
        <header data-tauri-drag-region className="drag-region flex shrink-0 items-center gap-3 border-b border-glass-edge px-3 pb-2 pt-4">
          ...
          <div className="flex flex-1 items-center gap-2.5">
            ...
            <div data-tauri-drag-region="false" className="flex items-center gap-1.5 border-l border-white/10 pl-2 no-drag">
              ...
              <button
                onClick={async () => {
                  const nextPrimary = settings.routing.primary === "gemini" ? "openai" : "gemini";
                  await saveSettings({
                    ...settings,
                    routing: { ...settings.routing, primary: nextPrimary },
                  });
                }}
                className="flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[9.5px] font-mono border border-white/20 bg-neutral-900/90 hover:border-accent/60 text-white shadow-sm hover:text-white transition-colors cursor-pointer"
                title={`Switch Primary Provider (Current: ${settings.routing.primary})`}
              >
                <KeyRound className="h-2.5 w-2.5" />
                <span className="capitalize">{settings.routing.primary}</span>
              </button>
              ...
              >
                ...
                {listening && settings.audio.captureMicrophone && (
                  <div className="flex items-center h-1.5 w-6 bg-black/45 rounded border border-white/5 overflow-hidden ml-1">
                    <div 
                      className="h-full bg-accent transition-all duration-75 rounded-sm"
                      style={{ width: `${Math.min(100, Math.max(0, micLevel * 250))}%` }}
                    />
                  </div>
                )}
              </button>
              ...
              >
                ...
                {listening && settings.audio.captureSystemAudio && (
                  <div className="flex items-center h-1.5 w-6 bg-black/45 rounded border border-white/5 overflow-hidden ml-1">
                    <div 
                      className="h-full bg-blue-400 transition-all duration-75 rounded-sm"
                      style={{ width: `${Math.min(100, Math.max(0, systemLevel * 250))}%` }}
                    />
                  </div>
                )}
              </button>
            ...
            <div data-tauri-drag-region="false" className="flex items-center gap-1.5 text-[11px] font-medium ml-auto no-drag">
              {!settings.audio.captureMicrophone && !settings.audio.captureSystemAudio ? (
                ...
                <>
                  {sttStatus === "idle" && (
                    <span className="flex items-center gap-1 text-white/30" title="Audio engine is inactive (Click to Start Listening in dashboard/overlay)">
                      <span className="h-1.5 w-1.5 rounded-full bg-white/20" />
                      <span>Idle</span>
                    </span>
                  )}
                  {sttStatus === "listening" && (
                    <span className="flex items-center gap-1 text-emerald-400" title="Actively listening for speaking pauses">
                      <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />
                      <span>Listening</span>
                    </span>
                  )}
                  {sttStatus === "transcribing" && (
                    <span className="flex items-center gap-1 text-amber-400" title="Transcribing voice to text...">
                  ...
                  {sttStatus === "error" && (
                    <span 
                      className="flex items-center gap-1 text-danger cursor-pointer hover:underline" 
                      title={`${sttErrorMessage || "STT Error"} (Click for details)`}
                      onClick={() => alert(`STT Diagnostic Error:\n\n${sttErrorMessage || "Unknown transcription error. Check your API keys and internet connection."}`)}
                    >
              ...
              {answer?.firstTokenMs && (
                <span className="font-mono text-[10px] text-white/30 border-l border-white/10 pl-2 ml-1" title="Time to first token">
                        ...
                        {answer.question && (
                          <div className="mb-2 flex items-start justify-between text-[14px] font-medium text-white/80">
                            ...
                            <div className="flex items-center gap-1.5 ml-2 shrink-0">
                              <AnswerPacingTimer />
                              {answer.isCached && (
                                <span className="rounded bg-emerald-500/15 px-1.5 py-0.5 text-[10px] text-emerald-300 font-mono">
                                  ⚡ Cache (0 Token / $0)
                                </span>
                              )}
                              {!answer.isCached && answer.provider && answer.model && (
                                <span className="rounded bg-white/5 px-1.5 py-0.5 text-[10px] text-white/40 font-mono">
                        ...
                        {item.question && (
                          <div className="mb-2 flex items-start justify-between text-[14px] font-medium text-white/80">
                            ...
                            <div className="flex items-center gap-1.5 ml-2 shrink-0">
                              ...
                              {!item.isCached && item.provider && item.model && (
                                <span className="rounded bg-white/5 px-1.5 py-0.5 text-[10px] text-white/40 font-mono">
            ...
            <footer className="shrink-0 border-t border-glass-edge px-2 py-2">
              {mode === "ask" ? (
                ...
                <div className="flex flex-col gap-2 no-drag font-mono text-[10px]">
                  {/* Single Row: Badges, Selectors, and Action Buttons */}
                  <div className="flex items-center justify-between gap-2 w-full">
                    <div className="flex items-center gap-1.5 flex-wrap flex-1">
                      ...
                      <select
                        value={settings.audio.sttEngine ?? "auto"}
                        onChange={async (e) => {
                          const newSettings = { ...settings, audio: { ...settings.audio, sttEngine: e.target.value as any } };
                          await saveSettings(newSettings);
                          if (listening) {
                            await stopListening();
                        ...
                        className="rounded-md border border-white/20 bg-neutral-900/90 px-1 py-0.5 font-mono text-[9px] text-white shadow-sm hover:border-accent/60 focus:outline-none cursor-pointer [color-scheme:dark]"
                        title="Change STT Engine"
                      >
                      ...
                      <select
                        value={settings.autoRespond === "manual-only" ? "manual" : "auto"}
                        onChange={async (e) => {
                          const trigger = e.target.value === "manual" ? "manual-only" : "question-detected";
                          await saveSettings({
                            ...settings,
                            autoRespond: trigger,
                          });
                        ...
                        className="rounded-md border border-white/20 bg-neutral-900/90 px-1 py-0.5 font-mono text-[9px] text-white shadow-sm hover:border-accent/60 focus:outline-none cursor-pointer [color-scheme:dark]"
                        title="Send Mode: Auto Send on pause vs Manual Click Send"
                      >
                        <option value="auto">🚀 Auto-Send</option>
                        <option value="manual">🖐️ Manual-Send</option>
                      </select>
                    </div>
                      ...
                      {(streaming || isTyping) ? (
                        ...
                        settings.autoRespond === "manual-only" ? (
                          <button
                            ...
                            title="Generate suggested answer from transcript"
                          >
...
async function transcribe(wavBase64: string, language: string): Promise<string> {
  ...
  const tryGemini = async (): Promise<string> => {
    ...
    if (!gemini?.keyRef) {
      errors.push("Gemini disabled or key config missing.");
      return "";
    }
        ...
        const res = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`,
          {
            ...
            body: JSON.stringify({
              contents: [{
                parts: [
                  { text: "Verbatim spoken audio transcript. Output verbatim text only." },
  ...
  const tryOpenAI = async (): Promise<string> => {
    ...
    if (!openai?.keyRef) {
      errors.push("OpenAI disabled or key config missing.");
      return "";
    }
  ...
  } else if (sttEngine === "openai-whisper") {
